export type IconName = 'menu'|'search'|'tune'|'compass'|'mountain'|'walk'|'city'|'sun'|'more'|'heart'|'pin'|'arrow'|'home'|'explore'|'profile'|'sparkle'|'camera'|'layers'|'locate'|'bookmark'|'folder'|'plane'|'calendar'|'check'|'wallet'|'document'|'bell'|'plus';
